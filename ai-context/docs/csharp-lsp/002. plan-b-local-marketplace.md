...
> **对比 [方案 A](./001. plan-a-shell-hack.md)**：步骤多 4–5 步，但稳定性显著优于壳 hack。
...
### 5.1 Roslyn LSP 项目加载机制（源码级）

正确配置 `args` 需要懂"什么时候加载几个项目"。下面基于 `external/roslyn`（dotnet/roslyn 仓库）源码。

#### 5.1.1 Workspace 是单例

```@e:/claude-hub/claude-code/external/roslyn/src/LanguageServer/Microsoft.CodeAnalysis.LanguageServer/HostWorkspace/LanguageServerWorkspaceFactory.cs:51-58
...
整个 server 进程只有 1 个 `HostWorkspace` + 1 个 `MiscellaneousFilesWorkspace`（游离文件）。所有 csproj 共存于 `HostWorkspace` 中，跨 csproj 的 `findReferences` / `goToDefinition` 能正常工作。

#### 5.1.2 `solution/open` 与 `project/open` 都是**追加语义**
...
**没有任何 close / unload 逻辑**——它直接 `BeginLoadingProjectAsync` 把 sln 里每个 csproj 加进当前 workspace。`SolutionPath` 字段被覆盖，但**已加载的 csproj 不会被卸载**。

理论上多次调用 `solution/open` 可以"合并"多个 sln 的 csproj 到同一 workspace，**但 Claude Code / VSCode 等客户端都不会主动调多次**——所以**用户视角下"多 sln 加载"不可用**。

#### 5.1.3 `--autoLoadProjects` 决策树

```@e:/claude-hub/claude-code/external/roslyn/src/LanguageServer/Microsoft.CodeAnalysis.LanguageServer/HostWorkspace/AutoLoadProjectsInitializer.cs:62-94
...
| # | 触发条件 | 实际加载 |
|---|---------|---------|
| 1 | `.vscode/settings.json` 里 `dotnet.defaultSolution: "disable"` | **不加载** |
| 2 | `.vscode/settings.json` 里 `dotnet.defaultSolution: "<path>"` | `OpenSolutionAsync(<path>)` |
| 3 | 单 workspaceFolder + 根目录**恰好** 1 个 `.sln` / `.slnx`（`TopDirectoryOnly` 不递归） | `OpenSolutionAsync(那一个)` |
| 4 | **其它所有情况**（根 0 个或 ≥2 个 sln，multi-root workspace） | **fallback**：`SearchOption.AllDirectories` 递归扫所有 `*.csproj` → `OpenProjectsAsync(全部)` |

**关键陷阱**：

- 根目录 ≥2 个 sln → **不**加载任一个，直接 fallback 扫全部 csproj
- 根目录无 sln（只在子目录有）→ fallback
- VSCode multi-root workspace → fallback

#### 5.1.4 OOM 真正诱因

不是"sln 数量"或"sln 大"，而是 **fallback 路径下 `SearchOption.AllDirectories` 把整个目录树的 csproj 全部加载**。

| 仓库类型 | 触发分支 | 加载数 | OOM 风险 |
|---------|---------|-------|---------|
| 标准 .NET 项目（根目录 1 个 sln） | 分支 3 | sln 列出的 csproj 数 | 低（除非巨型 sln，如 dotnet/roslyn 自身数百 csproj） |
| Unity 项目（根 1 个 `Assembly-CSharp.sln`） | 分支 3 | sln 列出的所有 asmdef csproj（几十个） | 中 |
| Mono-repo（根 ≥2 个 sln） | **分支 4 fallback** | **目录树所有** csproj | 🔴 高 |
| 纯 csproj 仓库（根无 sln） | **分支 4 fallback** | 目录树所有 csproj | 🔴 高 |
| 含 `node_modules/` 等无关 csproj 的仓库 | **分支 4 fallback** | 包括误扫的 csproj | 🔴 高 |

#### 5.1.5 推荐配置（按情况选）

**情况 A**：标准单 sln 项目，根目录恰好 1 个 sln  
→ 用 `--autoLoadProjects`（走分支 3，正确加载那个 sln）

**情况 B**：根目录有多 sln、无 sln、或想精确控制范围  
→ **删** `--autoLoadProjects`，**加** `--solutionPath` 显式指定：
...
```json
{
  "roslyn": {
    ...
    "args": [
      "--stdio",
      "--solutionPath", "${HOME}/repos/MyApp/Backend.sln",
      "--logLevel", "Information",
...
**情况 C**：跨多个 sln 分析（罕见）  
→ Claude Code 客户端不会主动调多次 `solution/open`，**协议层可行但客户端不支持**。退路：选主 sln，跨 sln 引用用 `GrepTool` / `FileGlobTool` 兜底。

**注意**：`--solutionPath` 也走 `OpenSolutionAsync`，所以加载范围是"sln 里列出的全部 csproj"。如果你的 sln 本身就列了几百个 csproj，仍可能 OOM——这种情况只能改造 sln 或用 `--projectPath` 显式列出需要的 csproj 子集。
...
| `--autoLoadProjects` fallback OOM（根多 sln / 无 sln → 扫全部 csproj） | � 高 | 删 `--autoLoadProjects`，加 `--solutionPath` 显式指定（[§5.1.5](#515-推荐配置按情况选)） |
...
- Claude Code 源码位置：
  ...
  - `@/e:/claude-hub/claude-code/src/utils/plugins/lspPluginIntegration.ts:255-275` — `resolvePluginLspEnvironment` 变量替换
  ...
  - `@/e:/claude-hub/claude-code/src/services/lsp/LSPServerManager.ts:121-137` — server instance 创建 + `workspace/configuration` 内置响应
  - `@/e:/claude-hub/claude-code/src/services/lsp/LSPServerManager.ts:189-208` — `getServerForFile` first-wins 路由
  - `@/e:/claude-hub/claude-code/src/services/lsp/LSPServerManager.ts:217-235` — `ensureServerStarted` lazy spawn
  - `@/e:/claude-hub/claude-code/src/services/lsp/LSPClient.ts:98` — 真正 `spawn` 子进程的位置
  - `@/e:/claude-hub/claude-code/src/services/lsp/manager.ts:100-110` — `isLspConnected` 判定
...
- dotnet/roslyn 源码位置（`external/roslyn`）：
  - `@/e:/claude-hub/claude-code/external/roslyn/src/LanguageServer/Microsoft.CodeAnalysis.LanguageServer/HostWorkspace/LanguageServerWorkspaceFactory.cs:51-58` — Workspace 单例
  - `@/e:/claude-hub/claude-code/external/roslyn/src/LanguageServer/Microsoft.CodeAnalysis.LanguageServer/HostWorkspace/LanguageServerProjectSystem.cs:55-80` — `OpenSolutionAsync` / `OpenProjectsAsync` 追加语义
  - `@/e:/claude-hub/claude-code/external/roslyn/src/LanguageServer/Microsoft.CodeAnalysis.LanguageServer/HostWorkspace/OpenSolutionHandler.cs:13-41` — `solution/open` LSP handler（单参数）
  - `@/e:/claude-hub/claude-code/external/roslyn/src/LanguageServer/Microsoft.CodeAnalysis.LanguageServer/HostWorkspace/OpenProjectsHandler.cs:14-42` — `project/open` LSP handler（数组参数）
  - `@/e:/claude-hub/claude-code/external/roslyn/src/LanguageServer/Microsoft.CodeAnalysis.LanguageServer/HostWorkspace/AutoLoadProjectsInitializer.cs:30-118` — `--autoLoadProjects` 完整决策树
- 上游 issue / 资源：
...
- 相邻文档：
  - [`plan-b-quickstart.md`](./003. plan-b-quickstart.md) — 5 分钟快速配置版
  - [`plan-a-shell-hack.md`](./001. plan-a-shell-hack.md) — 壳 hack 方案
