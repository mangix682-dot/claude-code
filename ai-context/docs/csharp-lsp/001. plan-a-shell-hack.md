...
> **推荐替代**：生产环境用 [`plan-b-local-marketplace.md`](./002. plan-b-local-marketplace.md)。
...
如果以上都对但 Claude 调用 LSP 工具挂起 / 返回空——是 Roslyn LS 的客户端协议兼容问题（`workspace/configuration`、`client/registerCapability`、`window/workDoneProgress/create`），见 [`csharp-lsp-adapter.md`](./csharp-lsp-adapter.md)（若有）或直接换 [方案 B 配 CSharpLspAdapter](./002. plan-b-local-marketplace.md#csharplspadapter-集成)。
